...
func (stdnt *Student) New(ctx context.Context) error {
	pKey := ParentKey(ctx)
...
func Current(ctx context.Context, current bool) (*Student, error) {
	curU, err := user.Current(ctx)
	if err != nil {
		return nil, err
	}
	stdnt, err := WithEmail(ctx, curU.Email, current)
...
func WithEmail(ctx context.Context, email string, current bool) (*Student, error) {
...
func All(ctx context.Context, current bool) ([]*Student, error) {
	ancestor := ParentKey(ctx)
...
func ParentKey(ctx context.Context) *datastore.Key {
	return datastore.NewKey(ctx, "Student", "Release", 0, nil)
}
